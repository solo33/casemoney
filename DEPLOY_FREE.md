...
## Почта через Brevo на Render Free

Бесплатный Render блокирует SMTP-порты, поэтому коды регистрации отправляются
через Brevo HTTPS API. Создайте и подтвердите отправителя в Brevo, затем добавьте
в Environment Group backend-сервиса:

- `BREVO_API_KEY` — ключ API;
- `BREVO_SENDER_EMAIL` — подтверждённый адрес отправителя;
- `BREVO_SENDER_NAME` — `CaseMoney`.

Если `BREVO_API_KEY` задан, приложение автоматически использует Brevo вместо
SMTP. Ключ нельзя добавлять в репозиторий или передавать в переписке.
