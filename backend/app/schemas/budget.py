from app.money import MoneyValue
from datetime import date, datetime
from typing import Literal, Optional

from pydantic import BaseModel, Field


class BudgetCreate(BaseModel):
    category_id: int
    amount: MoneyValue = Field(gt=0)
    currency: str = Field(min_length=2, max_length=10)
    period: Literal["month", "quarter", "year"] = "month"
    period_start: Optional[date] = None
    rollover_mode: Literal["none", "carry_remaining", "carry_balance"] = "none"
    include_planned: bool = False
    daily_amount: Optional[MoneyValue] = Field(None, ge=0)
    scope: Literal["personal", "family", "mixed"] = "personal"


class BudgetUpdate(BaseModel):
    amount: Optional[MoneyValue] = Field(None, gt=0)
    rollover_mode: Optional[Literal["none", "carry_remaining", "carry_balance"]] = None
    include_planned: Optional[bool] = None
    daily_amount: Optional[MoneyValue] = Field(None, ge=0)
    scope: Optional[Literal["personal", "family", "mixed"]] = None


class BudgetResponse(BaseModel):
    id: int
    category_id: int
    category_name: str
    category_icon: Optional[str] = None
    period: str
    period_start: date
    amount: MoneyValue
    effective_limit: MoneyValue
    currency: str
    spent: MoneyValue
    remaining: MoneyValue
    percent: MoneyValue
    is_overspent: bool
    rollover_mode: str
    carry_in: MoneyValue
    include_planned: bool
    daily_amount: Optional[MoneyValue]
    expected_spent_to_date: Optional[MoneyValue]
    daily_deviation: Optional[MoneyValue]
    scope: str
    created_at: datetime
    updated_at: datetime

    class Config:
        from_attributes = True


class BudgetSuggestion(BaseModel):
    category_id: int
    category_name: str
    category_icon: Optional[str] = None
    average_amount: MoneyValue
    currency: str
    months_with_data: int
