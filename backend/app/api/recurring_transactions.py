...
def list_recurring_transactions(db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
...
def create_recurring_transaction(data: RecurringTransactionCreate, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
...
def update_recurring_transaction(recurring_id: int, data: RecurringTransactionUpdate, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
...
def skip_next_recurring_transaction(recurring_id: int, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
...
def finish_recurring_transaction(recurring_id: int, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
...
def recurring_transaction_runs(recurring_id: int, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
    return operation_response(queries.recurring_transaction_runs(recurring_id=recurring_id, db=db, user_id=user_id))


@router.delete("/{recurring_id}", status_code=204)
def delete_recurring_transaction(recurring_id: int, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
