...
def list_tags(db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
    return operation_response(queries.list_tags(db=db, user_id=user_id))


@router.post("/", response_model=TagResponse, status_code=201)
def create_tag(data: TagCreate, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
    return operation_response(commands.create_tag(data=data, db=db, user_id=user_id))


@router.patch("/{tag_id}", response_model=TagResponse)
def update_tag(tag_id: int, data: TagUpdate, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
    return operation_response(commands.update_tag(tag_id=tag_id, data=data, db=db, user_id=user_id))


@router.delete("/{tag_id}", status_code=204)
def delete_tag(tag_id: int, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
    return operation_response(commands.delete_tag(tag_id=tag_id, db=db, user_id=user_id))


@router.get("/{tag_id}/report")
def tag_report(tag_id: int, db: Session = Depends(financial_db, scope="function"), user_id: int = Depends(get_current_user_id)):
