...
def tag_report(tag_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    ...
    return queries.tag_report(tag_id=tag_id, db=db, user_id=user_id)
