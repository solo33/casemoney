...
def overview(db: Session = Depends(financial_db, scope="function"), user: User = Depends(current_user)):
...
def checkout(data: CheckoutRequest, db: Session = Depends(financial_db, scope="function"), user: User = Depends(current_user)):
...
def refresh_payment(db: Session = Depends(financial_db, scope="function"), user: User = Depends(current_user)):
...
def cancel(db: Session = Depends(financial_db, scope="function"), user: User = Depends(current_user)):
    return operation_response(commands.cancel(db=db, user=user))


@router.post("/resume", response_model=BillingActionResponse)
def resume(db: Session = Depends(financial_db, scope="function"), user: User = Depends(current_user)):
    return operation_response(commands.resume(db=db, user=user))


@router.post("/webhook/yookassa", include_in_schema=False)
async def yookassa_webhook(request: Request, db: Session = Depends(financial_db, scope="function")):
