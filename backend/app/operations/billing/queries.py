...
def plans(user: User=None):
    return [
        ...
        PlanResponse(code="family", name="Family", price=decimal(yookassa.family_price()), period="month", current=user.plan == "family"),
    ]
...
def overview(db: Session=None, user: User=None):
    ...
    return BillingOverview(
        ...
        plan_expires_at=user.plan_expires_at, family_price=decimal(yookassa.family_price()), subscription=subscription,
        payments=payments, family_upgrade_enabled=user.family_upgrade_enabled,
        billing_enabled=app_config_svc.is_billing_enabled(db),
        test_month_price=decimal(_test_price("month")), test_year_price=decimal(_test_price("year")),
    )
