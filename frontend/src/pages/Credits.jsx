...
export default function Credits({ scope = "debt" }) {
  ...
  const [payment, setPayment] = useState({ amount: "", account_id: "", notes: "", is_early_payment: false });
  ...
  const openPayment = (item, early = false) => { setPaying(item); setPayment({ amount: early ? "" : item.monthly_payment || "", account_id: item.source_account_id || "", notes: "", is_early_payment: early }); };
  const submitPayment = async event => {
    event.preventDefault(); setBusy(true); setError("");
    try {
      await api.post(`/api/credits/${paying.id}/payments`, { amount: Number(payment.amount), account_id: Number(payment.account_id), notes: payment.notes || null, is_early_payment: Boolean(payment.is_early_payment) });
...
function CreditForm({ form, setForm, editingId, busy, accounts, categories, currencies, kindOptions, onSubmit, onCancel }) {
  ...
  return <section className="credit-form-card"><div className="credit-section-title"><h2>{editingId ? "Изменить запись" : "Новая запись"}</h2><button type="button" className="btn-ghost" onClick={onCancel}>×</button></div><form onSubmit={onSubmit} className="credit-form">
    ...
    {form.kind === "mortgage" && <Field label="Годовая ставка, %"><input type="number" min="0" max="100" step="0.01" value={form.annual_interest_rate} onChange={e => setForm({ ...form, annual_interest_rate: e.target.value })} /><small className="credit-field-hint">При оплате система отделит проценты от погашения тела кредита.</small></Field>}
...
function MortgageOverview({ items }) {
  ...
  return <section className="mortgage-overview">
    <h2>Прогресс по ипотеке</h2>
    <p>Досрочный платёж уменьшает тело кредита и сокращает срок по этому расчёту.</p>
    {items.map(item => {
      ...
      return <article key={item.id}>
        ...
        <small>{months ? `При текущем платеже — ещё примерно ${months} мес.` : "Укажите регулярный платёж, чтобы увидеть срок."}</small>
      </article>;
    })}
  </section>;
}
