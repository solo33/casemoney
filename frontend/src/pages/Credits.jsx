...
export default function Credits() {
  ...
  return <main className="page credits-page">
    ...
    {loading ? <p>Обновляем данные…</p> : active.length === 0 && !showForm ? <section className="credit-empty"><h2>Пока нет обязательств и депозитов</h2><p>Добавьте будущий платёж или доход — CaseMoney покажет ближайшую дату и напомнит о ней.</p><button onClick={openCreate}>Добавить первый</button></section> : <div className="credits-grid">{active.map(item => <CreditCard key={item.id} item={item} busy={busy} onPay={openPayment} onEdit={openEdit} onClose={() => setStatus(item, "closed")} />)}</div>}
    {closed.length > 0 && <details className="closed-credits"><summary>Закрытые обязательства ({closed.length})</summary><div className="credits-grid">{closed.map(item => <CreditCard key={item.id} item={item} busy={busy} onEdit={openEdit} onRestore={() => setStatus(item, "active")} />)}</div></details>}
...
function CreditForm({ form, setForm, editingId, busy, accounts, categories, currencies, onSubmit, onCancel }) {
  ...
  return <section className="credit-form-card"><div className="credit-section-title"><h2>{editingId ? "Изменить обязательство" : "Новое обязательство"}</h2><button type="button" className="btn-ghost" onClick={onCancel}>×</button></div><form onSubmit={onSubmit} className="credit-form">
    ...
    {!editingId && form.kind === "private_debt" && <Field label="Направление"><select value={form.direction} onChange={e => setForm({ ...form, direction: e.target.value })}><option value="owe">Я должен</option><option value="receivable">Мне должны</option></select></Field>}
...
function CreditCard({ item, busy, onPay, onEdit, onClose, onRestore }) {
