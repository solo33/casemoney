    ...
    <div className="home-layout" style={{
      ...
      <aside style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <Card>
          ...
          <div style={{ fontSize: 30, fontWeight: 700, color: "#22c55e", lineHeight: 1.1 }}>
            {formatMoney(totalBalance)} <span style={{ fontSize: 16, color: "#64748b", fontWeight: 500 }}>{mainCurrency}</span>
          </div>
          {byCurrency.length > 0 && (
        ...
        <Card noPadding>
          <div style={{ padding: "12px 16px 8px", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            ...
            <Link to="/accounts" style={{ fontSize: 12, color: "#6366f1", textDecoration: "none" }}>
              Настроить →
            </Link>
          </div>
          {grouped.length === 0 ? (
            <p style={{ padding: "10px 16px 16px", color: "#94a3b8", fontSize: 13 }}>
      ...
      <main style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* Records today + чарт */}
        <Card noPadding>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid #f1f5f9", display: "flex", gap: 12, alignItems: "baseline" }}>
          ...
          {todayTx.length === 0 ? (
            <p style={{ padding: 24, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>
        ...
        <Card>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
            ...
            <Link to="/reports" style={{ fontSize: 12, color: "#6366f1", textDecoration: "none" }}>
              Подробнее →
            </Link>
          </div>
          {monthExpenses.length === 0 ? (
            <p style={{ color: "#94a3b8", fontSize: 14 }}>Нет расходов за этот месяц</p>
          ) : (
            <>
              ...
              <div style={{
                ...
                <span style={{ color: "#475569" }}>Итого</span>
                <span style={{ color: "#ef4444" }}>{formatMoney(expenseTotal)} {sym}</span>
              </div>
            </>
          )}
        </Card>
...
function MonthBars({ points, sym }) {
  if (!points.length) {
    return <p style={{ color: "#94a3b8", fontSize: 14 }}>Нет данных</p>;
  }
      ...
      {points.map((p, idx) => {
        ...
        return (
          <div key={p.month} style={{
            ...
            <Bar value={p.income} max={maxVal} color="#22c55e" sym={sym} sign="+" />
            <Bar value={p.expense} max={maxVal} color="#ef4444" sym={sym} sign="−" />
          </div>
        );
      })}
    </div>
        ...
        <div style={{ marginTop: 2, marginLeft: 16 }}>
          {balances.map(b => (
            <div key={b.currency} style={{
              display: "flex", justifyContent: "space-between",
              fontSize: 12, color: "#64748b", padding: "1px 0",
            }}>
              <span style={{ color: "#94a3b8" }}>{b.currency}</span>
...
function CategoryBar({ name, icon, color, total, max, sym }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        height: 14, background: "#f8fafc", borderRadius: 3, overflow: "hidden",
      }}>
        <div style={{
      ...
      <span style={{ color: "#475569", minWidth: 80, textAlign: "right", fontWeight: 500 }}>
